...
{
    [super viewDidLoad];
    cityCodeList = CITY_CODE_LIST_ARR ;
    cityList = CITY_LIST_ARR ;
    
    postCityParams = [[NSMutableDictionary alloc] init];
...
-(void)fetchStoreData{
    ...
    [postCityParams setObject:[NSString stringWithFormat:@"%d",pageOffset] forKey:@"offset"];
    [postCityParams setObject:[NSString stringWithFormat:@"%d",storeCategoryId] forKey:@"categoryId"];
    ...
    [connections sendRequestWithPath:STORE_SCREEN andParameters:postCityParams showLoader:isLoading];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView.tag == 101) {
        ...
        self.cityView.hidden = YES;
       cityCode = [[NSString alloc] initWithFormat:@"%@",[cityCodeList objectAtIndex:indexPath.row]];
        NSLog(@"%@",[cityList objectAtIndex:indexPath.row]);
        [postCityParams setObject:[NSString stringWithFormat:@"%@",cityCode] forKey:@"cityId"];
        [_cityBtn setTitle:[cityList objectAtIndex:indexPath.row] forState:UIControlStateNormal];
         pageOffset = 1;
        [self fetchStoreData];
    }
}
...
- (void)flowView:(WaterflowView *)_flowView willLoadData:(int)page{
    pageOffset = page;
    [self fetchStoreData];
    //make a judegement to decide whether to call reloadData or reload Failed
}
