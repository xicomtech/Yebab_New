...
@synthesize currentLoginController;
 ...
 NSString* TWITTER_OAUTH_VERIFIER;
...
- (void) verifyTwitterAuthorizationToken:(NSString *) oauth_verifier
    ...
    NSLog(@"self.oauth_token: %@, oauthverifier: %@",self.oauth_token,oauth_verifier);
    NSDictionary* params = [NSDictionary dictionaryWithObjectsAndKeys: 
                            self.oauth_token,@"oauth_token",
                            oauth_verifier,@"oauth_verifier",nil];
    self.oauth_verifier = [[NSString alloc] initWithString:oauth_verifier];
    TWITTER_OAUTH_TOKEN  = [[NSString alloc] initWithString:self.oauth_token];
    TWITTER_OAUTH_SECRET = [[NSString alloc] initWithString:self.oauth_token_secret];
    TWITTER_OAUTH_VERIFIER = [[NSString alloc] initWithString:oauth_verifier];
...
- (void) validateTwitterCredentialsWithCompletition:(DMOAuthTwitterCredentialValidationResult) completition {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        completition(([response statusCode] == 200),[NSJSONSerialization JSONObjectWithData:data options:kNilOptions error:nil]);//[resultData objectFromJSONString]);
    });
}
...
- (void) logout {
    ...
    if (currentLoginController != nil) {
        [currentLoginController dismissViewControllerAnimated:YES completion:nil];
