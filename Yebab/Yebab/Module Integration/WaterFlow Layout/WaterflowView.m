...
{    
    ...
    for (int i = 0; i< numberOfColumns; i++)
    {
        ...
        NSInteger rows = [_flowdatasource flowView:self numberOfRowsInColumn:i];
        
        CGFloat columHeight = 0.f;
        for (int j =0; j < rows; j++)
        {
            CGFloat height = [_flowdelegate flowView:self heightForRowAtIndexPath:[NSIndexPath indexPathForRow:j inSection:i]]+10;
            columHeight += height;
    ...
    //scrollHeight = [self customInit];

    self.contentSize = CGSizeMake(self.frame.size.width, scrollHeight + LOADINGVIEW_HEIGHT);
    
    self.loadFooterView.frame = CGRectMake(0, scrollHeight - 10, self.frame.size.width, LOADINGVIEW_HEIGHT);
    [self pageScroll];
...
-(CGFloat)customInit{
    ...
    NSInteger _cellCount = [_flowdatasource flowView:self numberOfRowsInColumn:0];
    ...
    for (int i = 0; i< _cellCount ; i++)
    {
        ...
        if(self.cellHeight.count < numberOfColumns)
        {
            ...
            [self.cellHeight addObject:[NSMutableArray arrayWithObject:[NSNumber numberWithFloat:[_flowdelegate flowView:self heightForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]]]]];
            //[self.cellIndex addObject:[NSMutableArray arrayWithObject:[NSNumber numberWithInt:i]]];
            minHeight = [_flowdelegate flowView:self heightForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]];
            minHeightAtColumn ++;
            continue;
 }
        ...
        [[self.cellHeight objectAtIndex:minHeightAtColumn] addObject:[NSNumber numberWithFloat:minHeight+=[_flowdelegate flowView:self heightForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]]]];
       ...
       // [self.cellPosition addObject:[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:(cellWidth+padding)*minHeightAtColumn],@"x",[NSNumber numberWithFloat:minHeight],@"y", nil]];
        ...
        CGFloat x = [[[self.cellPosition objectAtIndex:path.item] objectForKey:@"x"] floatValue]+5;
...
- (void)pageScroll
{
    ...
    for (int i = 0 ; i< numberOfColumns; i++)
    {
		...
		float width = 150.0f;//self.frame.size.width / numberOfColumns;
        ...
        if ([self.visibleCells objectAtIndex:i] == nil || ((NSArray*)[self.visibleCells objectAtIndex:i]).count == 0) //everytime reloadData is called and no cells in visibleCellArray
        {
			...
			cell.frame = CGRectMake(origin_x, origin_y, width, height-10);
...
{
    float bottomEdge = scrollView.contentOffset.y + scrollView.frame.size.height +246;
